 ...
 * @internal
 */
#[CoversClass(KeyPairCrudController::class)]
#[RunTestsInSeparateProcesses]
final class KeyPairCrudControllerTest extends AbstractEasyAdminControllerTestCase
{
    public function testUnauthenticatedAccessIsBlocked(): void
    {
        $client = self::createClient();
        $client->request('GET', '/aws-lightsail/key-pair');
    ...
    {
        $client = self::createClient();
        $client->request('POST', '/aws-lightsail/key-pair/new');
    ...
    {
        $client = self::createClient();
        $client->request('PUT', '/aws-lightsail/key-pair');
    ...
    {
        $client = self::createClient();
        $client->request('DELETE', '/aws-lightsail/key-pair/1/delete');
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 302, 401, 403, 404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('HEAD', '/aws-lightsail/key-pair');
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 401, 403, 404]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('OPTIONS', '/aws-lightsail/key-pair');
    ...
    public function testSearchFunctionality(): void
    {
        ...
        // 测试搜索功能（通过查询参数）
        $client->request('GET', '/aws-lightsail/key-pair', ['query' => 'test']);
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 401, 403, 404]);

        // 测试过滤功能（通过filter参数）
        $client->request('GET', '/aws-lightsail/key-pair', ['filters' => ['name' => 'test']]);
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 401, 403, 404]);

        // 测试排序功能
        $client->request('GET', '/aws-lightsail/key-pair', ['sort' => ['name' => 'ASC']]);
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 401, 403, 404]);
    }
    ...
    /**
     * @return AbstractCrudController<KeyPair>
     */
    protected function getControllerService(): AbstractCrudController
    {
        return self::getService(KeyPairCrudController::class);
    }
