 ...
 * @internal
 */
#[CoversClass(StaticIpCrudController::class)]
#[RunTestsInSeparateProcesses]
final class StaticIpCrudControllerTest extends AbstractEasyAdminControllerTestCase
{
    public function testUnauthenticatedAccessIsBlocked(): void
    {
        $client = self::createClient();
        $client->request('GET', '/aws-lightsail/static-ip');
    ...
    {
        $client = self::createClient();
        $client->request('POST', '/aws-lightsail/static-ip/new');
    ...
    {
        $client = self::createClient();
        $client->request('PUT', '/aws-lightsail/static-ip');
    ...
    {
        $client = self::createClient();
        $client->request('DELETE', '/aws-lightsail/static-ip/1/delete');
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 302, 401, 403, 404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('HEAD', '/aws-lightsail/static-ip');
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 401, 403, 404]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('OPTIONS', '/aws-lightsail/static-ip');
    ...
    public function testSearchFunctionality(): void
    {
        ...
        // 测试搜索功能（通过查询参数）
        $client->request('GET', '/aws-lightsail/static-ip', ['query' => 'test']);
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 401, 403, 404]);

        // 测试过滤功能（通过filter参数）
        $client->request('GET', '/aws-lightsail/static-ip', ['filters' => ['name' => 'test']]);
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 401, 403, 404]);

        // 测试排序功能
        $client->request('GET', '/aws-lightsail/static-ip', ['sort' => ['name' => 'ASC']]);
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 401, 403, 404]);
    }
    ...
    /**
     * @return AbstractCrudController<StaticIp>
     */
    protected function getControllerService(): AbstractCrudController
    {
        return self::getService(StaticIpCrudController::class);
    }
