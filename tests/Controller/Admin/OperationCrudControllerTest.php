 ...
 * @internal
 */
#[CoversClass(OperationCrudController::class)]
#[RunTestsInSeparateProcesses]
final class OperationCrudControllerTest extends AbstractEasyAdminControllerTestCase
{
    public function testUnauthenticatedAccessIsBlocked(): void
    {
        $client = self::createClient();
        $client->request('GET', '/aws-lightsail/operation');
    ...
    {
        $client = self::createClient();
        $client->request('POST', '/aws-lightsail/operation/new');
    ...
    {
        $client = self::createClient();
        $client->request('PUT', '/aws-lightsail/operation');
    ...
    {
        $client = self::createClient();
        $client->request('DELETE', '/aws-lightsail/operation/1/delete');
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 302, 401, 403, 404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('HEAD', '/aws-lightsail/operation');
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 401, 403, 404]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('OPTIONS', '/aws-lightsail/operation');
    ...
    public function testSearchFunctionality(): void
    {
        ...
        // 测试搜索功能（通过查询参数）
        $client->request('GET', '/aws-lightsail/operation', ['query' => 'test']);
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 401, 403, 404]);

        // 测试过滤功能（通过filter参数）
        $client->request('GET', '/aws-lightsail/operation', ['filters' => ['name' => 'test']]);
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 401, 403, 404]);

        // 测试排序功能
        $client->request('GET', '/aws-lightsail/operation', ['sort' => ['name' => 'ASC']]);
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 401, 403, 404]);
    }
    ...
    /**
     * @return AbstractCrudController<Operation>
     */
    protected function getControllerService(): AbstractCrudController
    {
        return self::getService(OperationCrudController::class);
    }
    ...
    public static function provideIndexPageHeaders(): iterable
    {
        ...
        yield 'resourceType' => ['resourceType'];
        yield 'type' => ['type'];
        yield 'status' => ['status'];
        yield 'region' => ['region'];
        yield 'errorCode' => ['errorCode'];
        yield 'errorDetails' => ['errorDetails'];
        yield 'completionTime' => ['completionTime'];
    }
