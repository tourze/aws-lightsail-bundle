 ...
 * @internal
 */
#[CoversClass(ContactMethodCrudController::class)]
#[RunTestsInSeparateProcesses]
final class ContactMethodCrudControllerTest extends AbstractEasyAdminControllerTestCase
{
    public function testUnauthenticatedAccessIsBlocked(): void
    {
        $client = self::createClient();
        $client->request('GET', '/aws-lightsail/contact-method');
    ...
    {
        $client = self::createClient();
        $client->request('POST', '/aws-lightsail/contact-method/new');
    ...
    {
        $client = self::createClient();
        $client->request('PUT', '/aws-lightsail/contact-method');
    ...
    {
        $client = self::createClient();
        $client->request('DELETE', '/aws-lightsail/contact-method/1/delete');
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 302, 401, 403, 404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('HEAD', '/aws-lightsail/contact-method');
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 401, 403, 404]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('OPTIONS', '/aws-lightsail/contact-method');
    ...
    public function testSearchFunctionality(): void
    {
        ...
        // 测试搜索功能（通过查询参数）
        $client->request('GET', '/aws-lightsail/contact-method', ['query' => 'test']);
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 401, 403, 404]);

        // 测试过滤功能（通过filter参数）
        $client->request('GET', '/aws-lightsail/contact-method', ['filters' => ['name' => 'test']]);
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 401, 403, 404]);

        // 测试排序功能
        $client->request('GET', '/aws-lightsail/contact-method', ['sort' => ['name' => 'ASC']]);
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 401, 403, 404]);
    }
    ...
    /**
     * @return AbstractCrudController<ContactMethod>
     */
    protected function getControllerService(): AbstractCrudController
    {
        return self::getService(ContactMethodCrudController::class);
    }
    ...
    public static function provideIndexPageHeaders(): iterable
    {
        yield 'name' => ['name'];
        yield 'type' => ['type'];
        yield 'contactEndpoint' => ['contactEndpoint'];
        yield 'status' => ['status'];
        yield 'region' => ['region'];
