...
final class DomainCrudControllerTest extends AbstractEasyAdminControllerTestCase
{
    public function testUnauthenticatedAccessIsBlocked(): void
    {
        ...
        $client->request('GET', '/aws-lightsail/domain');
    ...
    {
        $client = self::createClient();
        $client->request('POST', '/aws-lightsail/domain/new');
    ...
    {
        $client = self::createClient();
        $client->request('PUT', '/aws-lightsail/domain');
    ...
    {
        $client = self::createClient();
        $client->request('DELETE', '/aws-lightsail/domain/1/delete');
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 302, 401, 403, 404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('HEAD', '/aws-lightsail/domain');
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 401, 403, 404]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('OPTIONS', '/aws-lightsail/domain');
    ...
    public function testSearchFunctionality(): void
    {
        ...
        // 测试搜索功能（通过查询参数）
        $client->request('GET', '/aws-lightsail/domain', ['query' => 'test']);
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 401, 403, 404]);

        // 测试过滤功能（通过filter参数）
        $client->request('GET', '/aws-lightsail/domain', ['filters' => ['name' => 'test']]);
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 401, 403, 404]);

        // 测试排序功能
        $client->request('GET', '/aws-lightsail/domain', ['sort' => ['name' => 'ASC']]);
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 401, 403, 404]);
    }
    ...
    /**
     * @return AbstractCrudController<Domain>
     */
