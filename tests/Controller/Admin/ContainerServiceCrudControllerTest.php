 ...
 * @internal
 */
#[CoversClass(ContainerServiceCrudController::class)]
#[RunTestsInSeparateProcesses]
final class ContainerServiceCrudControllerTest extends AbstractEasyAdminControllerTestCase
{
    public function testUnauthenticatedAccessIsBlocked(): void
    {
        $client = self::createClient();
        $client->request('GET', '/aws-lightsail/container-service');
    ...
    {
        $client = self::createClient();
        $client->request('POST', '/aws-lightsail/container-service/new');
    ...
    {
        $client = self::createClient();
        $client->request('PUT', '/aws-lightsail/container-service');
    ...
    {
        $client = self::createClient();
        $client->request('DELETE', '/aws-lightsail/container-service/1/delete');
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 302, 401, 403, 404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('HEAD', '/aws-lightsail/container-service');
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 401, 403, 404]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('OPTIONS', '/aws-lightsail/container-service');
    ...
    public function testSearchFunctionality(): void
    {
        ...
        // 测试搜索功能（通过查询参数）
        $client->request('GET', '/aws-lightsail/container-service', ['query' => 'test']);
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 401, 403, 404]);

        // 测试过滤功能（通过filter参数）
        $client->request('GET', '/aws-lightsail/container-service', ['filters' => ['name' => 'test']]);
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 401, 403, 404]);

        // 测试排序功能
        $client->request('GET', '/aws-lightsail/container-service', ['sort' => ['name' => 'ASC']]);
        $this->assertContains($client->getResponse()->getStatusCode(), [200, 401, 403, 404]);
    }
    ...
    /**
     * @return AbstractCrudController<ContainerService>
     */
    protected function getControllerService(): AbstractCrudController
    {
        return self::getService(ContainerServiceCrudController::class);
    }
    ...
    public static function provideIndexPageHeaders(): iterable
    {
        ...
        yield 'region' => ['region'];
        yield 'url' => ['url'];
        yield 'currentDeployment' => ['currentDeployment'];
        yield 'nextDeployment' => ['nextDeployment'];
        yield 'isPublicDomainEnabled' => ['isPublicDomainEnabled'];
        yield 'isPrivateDomainEnabled' => ['isPrivateDomainEnabled'];
        yield 'privateDomainName' => ['privateDomainName'];
        yield 'publicDomainNames' => ['publicDomainNames'];
        yield 'tags' => ['tags'];
    ...
    public static function provideNewPageFields(): iterable
    {
        ...
        yield 'currentDeployment' => ['currentDeployment'];
        yield 'nextDeployment' => ['nextDeployment'];
        yield 'containerImages' => ['containerImages'];
        yield 'isPublicDomainEnabled' => ['isPublicDomainEnabled'];
        yield 'isPrivateDomainEnabled' => ['isPrivateDomainEnabled'];
        yield 'privateDomainName' => ['privateDomainName'];
        yield 'publicDomainNames' => ['publicDomainNames'];
