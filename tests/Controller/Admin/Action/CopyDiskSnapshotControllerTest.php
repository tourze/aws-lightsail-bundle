...
#[CoversClass(CopyDiskSnapshotController::class)]
final class CopyDiskSnapshotControllerTest extends AbstractWebTestCase
{
    public function testUnauthenticatedAccessIsBlocked(): void
    {
        $client = self::createClient();
        $client->request('POST', '/admin/disk-snapshot/1/copy');

        // 预期：302 (认证重定向) 或 404 (路由不存在) 或 401/403 (未认证/未授权) 或 405 (方法不允许)
    ...
    {
        $client = self::createClient();
        $client->request('GET', '/admin/disk-snapshot/1/copy');
    ...
    {
        $client = self::createClient();
        $client->request('OPTIONS', '/admin/disk-snapshot/1/copy');
    ...
    {
        $client = self::createClient();
        $client->request('PUT', '/admin/disk-snapshot/1/copy');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('DELETE', '/admin/disk-snapshot/1/copy');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('PATCH', '/admin/disk-snapshot/1/copy');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('HEAD', '/admin/disk-snapshot/1/copy');
    ...
    {
        $client = self::createClient();
        $client->request($method, '/admin/disk-snapshot/1/copy');
