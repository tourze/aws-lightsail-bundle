...
#[CoversClass(SyncDiskController::class)]
final class SyncDiskControllerTest extends AbstractWebTestCase
{
    public function testUnauthenticatedAccessIsBlocked(): void
    {
        $client = self::createClient();
        $client->request('POST', '/admin/disk/1/sync');
    ...
    {
        $client = self::createClient();
        $client->request('GET', '/admin/disk/1/sync');
    ...
    {
        $client = self::createClient();
        $client->request('OPTIONS', '/admin/disk/1/sync');
    ...
    {
        $client = self::createClient();
        $client->request('PUT', '/admin/disk/1/sync');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('DELETE', '/admin/disk/1/sync');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('PATCH', '/admin/disk/1/sync');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('HEAD', '/admin/disk/1/sync');
    ...
    {
        $client = self::createClient();
        $client->request($method, '/admin/disk/1/sync');
