...
#[CoversClass(SyncContainerServiceController::class)]
final class SyncContainerServiceControllerTest extends AbstractWebTestCase
{
    public function testUnauthenticatedAccessIsBlocked(): void
    {
        $client = self::createClient();
        $client->request('POST', '/admin/container-service/1/sync');
    ...
    {
        $client = self::createClient();
        $client->request('GET', '/admin/container-service/1/sync');
    ...
    {
        $client = self::createClient();
        $client->request('OPTIONS', '/admin/container-service/1/sync');
    ...
    {
        $client = self::createClient();
        $client->request('PUT', '/admin/container-service/1/sync');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('DELETE', '/admin/container-service/1/sync');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('PATCH', '/admin/container-service/1/sync');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('HEAD', '/admin/container-service/1/sync');
    ...
    {
        $client = self::createClient();
        $client->request($method, '/admin/container-service/1/sync');
