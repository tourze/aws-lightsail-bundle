...
#[CoversClass(SyncDatabaseSnapshotController::class)]
final class SyncDatabaseSnapshotControllerTest extends AbstractWebTestCase
{
    public function testUnauthenticatedAccessIsBlocked(): void
    {
        $client = self::createClient();
        $client->request('POST', '/admin/database-snapshot/1/sync');
    ...
    {
        $client = self::createClient();
        $client->request('GET', '/admin/database-snapshot/1/sync');
    ...
    {
        $client = self::createClient();
        $client->request('OPTIONS', '/admin/database-snapshot/1/sync');
    ...
    {
        $client = self::createClient();
        $client->request('PUT', '/admin/database-snapshot/1/sync');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('DELETE', '/admin/database-snapshot/1/sync');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('PATCH', '/admin/database-snapshot/1/sync');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('HEAD', '/admin/database-snapshot/1/sync');
    ...
    {
        $client = self::createClient();
        $client->request($method, '/admin/database-snapshot/1/sync');
