...
#[CoversClass(TestAlarmController::class)]
final class TestAlarmControllerTest extends AbstractWebTestCase
{
    ...
    {
        $client = self::createClient();
        $client->request('OPTIONS', '/admin/alarm/1/test');
    ...
    {
        $client = self::createClient();
        $client->request('PUT', '/admin/alarm/1/test');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('DELETE', '/admin/alarm/1/test');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('PATCH', '/admin/alarm/1/test');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('HEAD', '/admin/alarm/1/test');
