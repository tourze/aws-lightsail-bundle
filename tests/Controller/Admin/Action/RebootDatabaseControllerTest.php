...
#[CoversClass(RebootDatabaseController::class)]
final class RebootDatabaseControllerTest extends AbstractWebTestCase
{
    public function testUnauthenticatedAccessIsBlocked(): void
    {
        $client = self::createClient();
        $client->request('POST', '/admin/database/1/reboot');
    ...
    {
        $client = self::createClient();
        $client->request('GET', '/admin/database/1/reboot');
    ...
    {
        $client = self::createClient();
        $client->request('OPTIONS', '/admin/database/1/reboot');
    ...
    {
        $client = self::createClient();
        $client->request('PUT', '/admin/database/1/reboot');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('DELETE', '/admin/database/1/reboot');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('PATCH', '/admin/database/1/reboot');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('HEAD', '/admin/database/1/reboot');
    ...
    {
        $client = self::createClient();
        $client->request($method, '/admin/database/1/reboot');
