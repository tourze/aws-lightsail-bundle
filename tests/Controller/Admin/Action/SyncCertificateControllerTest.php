...
#[CoversClass(SyncCertificateController::class)]
final class SyncCertificateControllerTest extends AbstractWebTestCase
{
    public function testUnauthenticatedAccessIsBlocked(): void
    {
        $client = self::createClient();
        $client->request('POST', '/admin/certificate/1/sync');
    ...
    {
        $client = self::createClient();
        $client->request('GET', '/admin/certificate/1/sync');
    ...
    {
        $client = self::createClient();
        $client->request('OPTIONS', '/admin/certificate/1/sync');
    ...
    {
        $client = self::createClient();
        $client->request('PUT', '/admin/certificate/1/sync');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('DELETE', '/admin/certificate/1/sync');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('PATCH', '/admin/certificate/1/sync');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('HEAD', '/admin/certificate/1/sync');
    ...
    {
        $client = self::createClient();
        $client->request($method, '/admin/certificate/1/sync');
