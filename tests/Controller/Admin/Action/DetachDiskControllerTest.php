...
#[CoversClass(DetachDiskController::class)]
final class DetachDiskControllerTest extends AbstractWebTestCase
{
    public function testUnauthenticatedAccessIsBlocked(): void
    {
        $client = self::createClient();
        $client->request('POST', '/admin/disk/1/detach');
    ...
    {
        $client = self::createClient();
        $client->request('GET', '/admin/disk/1/detach');
    ...
    {
        $client = self::createClient();
        $client->request('OPTIONS', '/admin/disk/1/detach');
    ...
    {
        $client = self::createClient();
        $client->request('PUT', '/admin/disk/1/detach');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('DELETE', '/admin/disk/1/detach');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('PATCH', '/admin/disk/1/detach');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('HEAD', '/admin/disk/1/detach');
    ...
    {
        $client = self::createClient();
        $client->request($method, '/admin/disk/1/detach');
