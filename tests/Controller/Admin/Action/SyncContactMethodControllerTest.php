...
#[CoversClass(SyncContactMethodController::class)]
final class SyncContactMethodControllerTest extends AbstractWebTestCase
{
    public function testUnauthenticatedAccessIsBlocked(): void
    {
        $client = self::createClient();
        $client->request('POST', '/admin/contact-method/1/sync');
    ...
    {
        $client = self::createClient();
        $client->request('GET', '/admin/contact-method/1/sync');
    ...
    {
        $client = self::createClient();
        $client->request('OPTIONS', '/admin/contact-method/1/sync');
    ...
    {
        $client = self::createClient();
        $client->request('PUT', '/admin/contact-method/1/sync');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('DELETE', '/admin/contact-method/1/sync');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('PATCH', '/admin/contact-method/1/sync');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('HEAD', '/admin/contact-method/1/sync');
    ...
    {
        $client = self::createClient();
        $client->request($method, '/admin/contact-method/1/sync');
