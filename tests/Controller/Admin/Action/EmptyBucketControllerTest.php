...
#[CoversClass(EmptyBucketController::class)]
final class EmptyBucketControllerTest extends AbstractWebTestCase
{
    public function testUnauthenticatedAccessIsBlocked(): void
    {
        $client = self::createClient();
        $client->request('POST', '/admin/bucket/1/empty');
    ...
    {
        $client = self::createClient();
        $client->request('GET', '/admin/bucket/1/empty');
    ...
    {
        $client = self::createClient();
        $client->request('OPTIONS', '/admin/bucket/1/empty');
    ...
    {
        $client = self::createClient();
        $client->request('PUT', '/admin/bucket/1/empty');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('DELETE', '/admin/bucket/1/empty');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('PATCH', '/admin/bucket/1/empty');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('HEAD', '/admin/bucket/1/empty');
    ...
    {
        $client = self::createClient();
        $client->request($method, '/admin/bucket/1/empty');
