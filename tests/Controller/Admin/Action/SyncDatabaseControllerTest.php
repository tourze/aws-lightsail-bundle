...
#[CoversClass(SyncDatabaseController::class)]
final class SyncDatabaseControllerTest extends AbstractWebTestCase
{
    public function testUnauthenticatedAccessIsBlocked(): void
    {
        $client = self::createClient();
        $client->request('POST', '/admin/database/1/sync');

        // 预期：302 (重定向到登录页) 或 401/403 (未认证/未授权) 或 404 (路由不存在) 或 405 (方法不允许)
        $statusCode = $client->getResponse()->getStatusCode();
        $this->assertContains($statusCode, [302, 401, 403, 404, 405], '未认证访问应该被阻止或重定向到登录页');
    }

    public function testPostMethodIsRequired(): void
    {
        $client = self::createClient();
        $client->request('GET', '/admin/database/1/sync');
    ...
    {
        $client = self::createClient();
        $client->request('OPTIONS', '/admin/database/1/sync');
    ...
    {
        $client = self::createClient();
        $client->request('PUT', '/admin/database/1/sync');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('DELETE', '/admin/database/1/sync');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('PATCH', '/admin/database/1/sync');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('HEAD', '/admin/database/1/sync');
    ...
    {
        $client = self::createClient();
        $client->request($method, '/admin/database/1/sync');
