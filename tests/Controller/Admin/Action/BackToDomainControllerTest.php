...
#[CoversClass(BackToDomainController::class)]
final class BackToDomainControllerTest extends AbstractWebTestCase
{
    public function testUnauthenticatedAccessIsBlocked(): void
    {
        $client = self::createClient();
        $client->request('POST', '/admin/domain/1/back');
    ...
    {
        $client = self::createClient();
        $client->request('GET', '/admin/domain/1/back');
    ...
    {
        $client = self::createClient();
        $client->request('OPTIONS', '/admin/domain/1/back');
    ...
    {
        $client = self::createClient();
        $client->request('PUT', '/admin/domain/1/back');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('DELETE', '/admin/domain/1/back');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('PATCH', '/admin/domain/1/back');
        $this->assertContains($client->getResponse()->getStatusCode(), [404, 405]);
    }
    ...
    {
        $client = self::createClient();
        $client->request('HEAD', '/admin/domain/1/back');
    ...
    {
        $client = self::createClient();
        $client->request($method, '/admin/domain/1/back');
